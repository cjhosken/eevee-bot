...
{command.name} {use}
    ...
    **Cooldown:** {0 if command._buckets._cooldown == None else command._buckets._cooldown.per} seconds
            ...
            ).add_field(name=EMPTY_CHAR, value=f"[Our Instagram]({INSTAGRAM_LINK}) | [Support Server]({SUPPORT_SERVER_LINK})", inline=False)
        return embed
    ...
    async def send_bot_help(self, mapping):# works at >help
        async with self.context.typing(): 
            ...
            try:
                for cogs, commands in mapping.items():
                    for command in commands:
                        if not command.hidden:
                            cmds.append(command.qualified_name.title())
                ...
                print(e)
            embed,view_ui = await self.get_bot_help(self.context,options,cmds)
            await self.context.reply(embed = embed,view=view_ui)

#################################################################################################################

    async def get_bot_help(cog, context,options,cmds): # gives the help cmd Embed
        ctx = context
        cmds = cmds
        ...
        embed = discord.Embed(
                title="WORKBENCH Bot", 
                colour=MAIN_COLOR, 
                description=f"{BOT_DESCRIPTION} \n```py\n try using {p}help <command> at each of the below sections```\n\n\n", timestamp=datetime.datetime.utcnow()
                )
